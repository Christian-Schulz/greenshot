...
3.  **Broadcast**: The `IpcSender` finds all running processes with the name `Greenshot`. For each of these processes, it attempts to connect to its named pipe.
...
-   `FirstLaunch`: A special command used internally on the first run after installation to trigger initial setup actions, like showing a welcome notification.
-   `ReloadConfig`: Instructs the running instance to reload its configuration from the `.ini` file.

## Command-line Handling

The `AppCommands` system is not only used for communication between different Greenshot processes but is also central to handling command-line arguments. When a new Greenshot instance is launched, it parses the command-line arguments and converts them into a series of `AppCommands`.
